...
class FirmwareUpgradeTest(unittest.TestCase):
    def setUp(self):
        ...
        self.backend.write_text(BACKEND.read_text().replace('/var/run/', str(self.root) + '/'))
        self.worker.write_text(WORKER.read_text().replace('/var/run/', str(self.root) + '/'))

    def run_backend(self, *args):
        return subprocess.check_output(['sh', str(self.backend), *args], text=True).strip()
    ...
    def test_asset_matching_requires_exact_known_device_and_variant(self):
        ...
        assets.write_text(
            'openwrt-airoha-an7581-gemtek_w1700k-ubi-squashfs-sysupgrade.itb\t17474367\tsha256:' + 'a' * 64 + '\thttps://example.com/ubi2.itb\n'
    ...
    def test_worker_rejects_missing_or_invalid_checksum_before_sysupgrade(self):
        for sha256 in ('', 'not-a-hash'):
            with self.subTest(sha256=sha256):
                result = subprocess.check_output(
                    ['sh', str(self.worker), 'test_validate', 'https://example.com/image.bin', 'image.bin', sha256, '0', '123'],
                    text=True,
                )
    ...
    def test_worker_rejects_path_traversal_and_invalid_size(self):
        ...
        for args in cases:
            with self.subTest(args=args):
                result = subprocess.check_output(['sh', str(self.worker), 'test_validate', *args], text=True)
                self.assertNotEqual(result.strip(), 'ok')
