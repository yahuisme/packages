...
(async()=>{
 ...
 try{
  ...
  const mloCSS=h.q('.mlo-map style').textContent;
  assert(mloCSS.includes('.mlo-map .mlo-summary-item strong{font-size:1.125em;font-weight:600}'),'explicit summary-value emphasis exception');
  assert(!/font-size|font-weight|text-transform|letter-spacing/.test(mloCSS.replace('.mlo-map .mlo-summary-item strong{font-size:1.125em;font-weight:600}','').replace('font-size:inherit;','')),'no typography overrides outside summary exception');
