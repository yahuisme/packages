...
async function boot({readonly=false,missing=false,frequencyFailure=false}={}) {
 ...
 if(process.env.WIFI7_LMO){
  const data=fs.readFileSync(process.env.WIFI7_LMO),index=data.readUInt32BE(data.length-4);
  assert(index<=data.length-4 && (data.length-4-index)%16===0);
  for(let p=index;p<data.length-4;p+=16){const key=data.readUInt32BE(p),offset=data.readUInt32BE(p+8),length=data.readUInt32BE(p+12);assert(offset+length<=index);w.TR[key.toString(16).padStart(8,'0')]=data.subarray(offset,offset+length).toString('utf8');}
 }
 w.eval(core);
...
async function main(){if(OUT)fs.mkdirSync(OUT,{recursive:true});const results=[];
 ...
 await test('numeric-apply-failure-and-retry',async h=>{h.field('radio0','power').value='24';h.state.applyCode=6;await h.clickSave();assert.deepEqual(h.applyRejections,[{type:'number',value:'6'}]);assert.equal(h.notifications.at(-1).text,h.w._('Failed to apply configuration: %s').replace('%s',h.w._('Permission denied')));assert.equal(h.field('radio0','power').value,'24');assert.equal(h.db().committed.radio0.txpower,undefined);const applies=h.calls.filter(c=>c.method==='apply').length;h.state.applyCode=0;await h.clickSave();assert.equal(h.calls.filter(c=>c.method==='apply').length,applies+1);assert.equal(h.db().committed.radio0.txpower,'24');});
 await test('ubus-apply-failure-and-retry',async h=>{h.field('radio0','power').value='25';h.state.applyCode=6;h.state.ubusFailure=true;await h.clickSave();assert.equal(h.notifications.at(-1).text,h.w._('Failed to apply configuration: %s').replace('%s',h.w._('Permission denied')));assert.equal(h.applyRejections[0].type,'object');assert.equal(h.db().committed.radio0.txpower,undefined);h.state.applyCode=0;await h.clickSave();assert.equal(h.db().committed.radio0.txpower,'25');});
