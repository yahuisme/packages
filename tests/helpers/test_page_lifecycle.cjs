...
const base=path.join(packagesRoot,'luci-app-homeproxy'),dir=process.env.LUCI_RESOURCE_DIR||path.resolve(__dirname,'../../../luci/modules/luci-base/htdocs/luci-static/resources');
async function pageBoot(name,translations={},options={}) {
...
const app=load('page',fs.readFileSync(base+'/htdocs/luci-static/resources/view/homeproxy/'+name+'.js','utf8'));for(let i=0;i<80;i++)await tick();const root=w.document.querySelector('#cbi-homeproxy');assert(root,name+' mounted');const map=mods.dom.findClassInstance(root);
 return {w,mods,app,map,calls,navigationErrors:h.navigationErrors,root:()=>w.document.querySelector('#cbi-homeproxy'),polls:()=>mods.poll.queue.map(x=>x.fn),setHandler:f=>handler=f};
}
