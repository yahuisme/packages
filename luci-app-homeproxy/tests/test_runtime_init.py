...
""")
            ...
            return x,(r/'sets').read_text() if (r/'sets').exists() else ''
    ...
    def test_success_and_no_change(self):
        x,sets=self.run_case('', '9095')
        self.assertEqual(x.returncode,0,x.stderr); self.assertEqual(sets,'')
        x,sets=self.run_case('', '')
        self.assertEqual(x.returncode,0,x.stderr); self.assertIn('homeproxy.config.dashboard_port',sets)
