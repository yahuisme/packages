...
const api=w.Function('dom','rpc','ui',s)(dom,rpc,ui),root=w.document.querySelector('.homeproxy-status');root.append(w.E('style',{},[api.css]),api.getConnectionStatus());
...
(async()=>{try{
 ...
 for(const [result,ms,expected,green]of [[true,0,'0 ms',true],[true,18,'18 ms',true],[true,null,'-',true],[false,18,'-',false],[null,null,'-',false],['true',18,'-',false]]){
  ...
  resolve({results:[{site:'baidu',result,latency_ms:ms}]});await new Promise(r=>setImmediate(r));
