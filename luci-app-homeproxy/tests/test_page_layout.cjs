...
const {translations,serve}=require('./homeproxy_browser_test_helpers.cjs');
const out=process.env.HP_LAYOUT_OUTPUT,aurora=process.env.AURORA_HTDOCS;
assert(out&&aurora,'Set HP_LAYOUT_OUTPUT and AURORA_HTDOCS');
const appdir=path.resolve(__dirname,'..'),tr=translations(appdir);
(async()=>{
 ...
 const server=serve(snapshots,aurora);await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;const rows=[],failures=[];
 try{browser=await chromium.launch({args:['--no-sandbox']});for(const width of [390,768,1440])for(const dark of [false,true]){
  for(const stage of Object.keys(snapshots)){
